...
word_dic = {'0': "I", '1': "V", '2': "X", '3': "L", '4': "C", '5': "D", '6': "M", '7':"A", '8':"B", '9':"c",'10':"D",'11':"E",'12':"F",'13':"G",
                '14':"H",'15':"I",'16':"J",'17':"K",'18':10**9,'19':5*10**9,'20':10**10,'21':5*10**10,'22':10**11,'23':5*10**11,'24':10**12,'25':5*10**12,
                ...
                '51':5*10**25,'52':10**26}
word_dic2 = {'0': 1, '1': 5, '2': 10, '3': 50, '4': 100, '5': 500, '6': 1000, '7':5*10**3, '8':10**4, '9':5*10**4,'10':10**5,'11':5*10**5,'12':10**6,'13':5*10**6,
...
class Solution1(object):
    def intToRoman(self, num: int) -> str:
           ...
           H = ['', 'H', 'HH', 'HHH', 'HI', 'I', 'IH', 'IHH', 'IHHH', 'HJ']  # 0, 10**7
           F = ['', 'F', 'FF', 'FFF', 'FG', 'G', 'GF', 'GFF', 'GFFF', 'FH']  # 0, 10**6
           D = ['', 'D', 'DD', 'DDD', 'DE', 'E', 'ED', 'EDD', 'EDDD', 'DF']  #0,10*5
           B = ['', 'B', 'BB', 'BBB', "Bc", "c", "cB", "cBB", 'cBBB', 'BD']  #0,10000,20000,30000,40000,50000...90000
           ...
           return  g[(num % 10**22)// 10**21]+g[(num % 10**21)// 10**20] + e[(num % 10**20)// 10**19] + c[(num % 10**19)// 10**18] + a[(num % 10**18)// 10**17]+Z[(num % 10**17)// 10**16] + x[(num % 10**16)// 10**15] + v[(num % 10**15)// 10**14] +T[(num % 10**14)// 10**13] +R[(num % 10**13)// 10**12]+P[(num % 10**12)// 10**11] +N[(num % 10**11)// 10**10] +l[(num % 10**10)// 10**9] + J[(num % 10**9)// 10**8] +H[(num % 10**8)// 10**7] +F[(num % 10**7)// 10**6] + D[(num % 10**6)// 10**5] + B[(num % 10**5)// 10**4] + M[num % 10000 // 1000] + C[(num % 1000) // 100] + X[(num % 100) // 10] + I[num % 10]
...
class Solution2(object):
    ...
    def romanToInt(self, s: str) -> int:
        ...
        if n <= 0:
            return 0
        merge_dic = {'I': 1, 'V': 5, 'X': 10, 'L': 50, 'C': 100, 'D': 500, 'M': 1000,"Z":5*10**16,"Y":10**16,"x":5*10**15,"W":10**15,"v":5*10**14,"U":10**14,"T":5*10**13,"S":10**13,"R":5*10**12,"Q":10**12,"P":5*10**11,"O":10**11,"N":5*10**10,"m":10**10,"l":5*10**9,"K":10**9,
                   "J":5*10**8,"i":10**8,"H":5*10**7,"G":5*10**6,"F":1000000,"E":500000,"d":100000,"c":50000,"B":10000,"A":5000,}
        i = 0
        ans = 0
        while (i < n - 1):
...
if len(input_list)<3 or len(input_list)>5:
...
def check_str(roman):
    ...
    index_roman9 = roman.find('VIV')

    if index_roman1 >= 0 or index_roman2 >= 0 or index_roman3 >= 0 or index_roman4 >= 0 or index_roman5 >= 0 or index_roman6 >=0 or index_roman7>=0 or index_roman8 >= 0 or index_roman9 >=0:
        return True
    else:
        return False
...
def convert(n):
    ...
    for i in range(len(rule_list)):
        rule_dic[rule_list[-1 - i]] = word_dic2[str(i)]
    if c.isdigit():#如果第一个字符串为数字
       ...
       return "".join(new_roman)
        ...
        convert_list = list(c)#需要被转换的字符串list
        # 要将第一个字符串转换成罗马数字
        convert_num = 0
        convert_roman_list = []#非标准转int
        convert_roman_list2 = []#int转roman
        for i in convert_list:
            if i in rule_dic:
                convert_roman_list2.append(roman.intToRoman(rule_dic[i]))
                convert_num = ans.romanToInt(''.join(convert_roman_list2))
                continue
            elif i in ARABIC_TO_ROMAN and i in rule_dic:
...
if check_pattern1(a, b, c):
    if c.isdigit():
        if check_num(c):
            ...
            if len(input_list)== 5:
                print("Sure! It is", convert(c))
            else:
                print("Sure! It is", roman.intToRoman(int(c)))
    elif type(c) == str:
        if check_str(c):
            print("Hey, ask me something that's not impossible to do!")
        else:
            if len(input_list) == 5:
                ...
                print("Sure! It is", ans.romanToInt((c)))
    else:
        print("I don't get what you want, sorry mate!")
