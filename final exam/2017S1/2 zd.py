...
def f(arg_for_seed, nb_of_elements, max_element):
    ...
    result_dict2= sorted(result_dict.keys())
 

    for first_letter in result_dict2:
        if result_dict[first_letter] ==1:
           print(f'{result_dict[first_letter]} element starts with {first_letter}')
        else:
           print(f'{result_dict[first_letter]} elements start with {first_letter}')

    #for index in range(0,len(result_dict)+1):
       # print(f'{result_dict.key[index]} element starts with {result_dict.value[index]}')
        #index +=1
